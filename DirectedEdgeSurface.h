...
class DirectedEdgeSurface
    ...
    std::map<unsigned int, unsigned int>addFlag;
    std::map<unsigned int, std::vector<unsigned int>> neighborVertex;

    void loopSubdivision();
    void updatePosition(unsigned int index);
    void computeNeighborVertex(unsigned int index);
    void computeNormal(unsigned int i);
    unsigned int next(unsigned int Edgeindex);
