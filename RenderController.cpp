...
RenderController::RenderController
    ...
    QObject::connect(   renderWindow->loopsizeSlider,	            SIGNAL(valueChanged(int)),
                        this,                                       SLOT(loopsizeChanged(int)));
...
void RenderController::loopsizeChanged(int value)
    { // RenderController::vertexSizeChanged()
    // reset the model's y translation (slider ticks are 1/512 each)
    renderParameters->loopsize = float(value) / 2;

    for(int i = 0;i<renderParameters->loopsize * 2;i++){
        directedEdgeSurface->loopSubdivision();
        std::ofstream outfile;
        outfile.open(renderParameters->path);
        directedEdgeSurface->WriteObjectStream(outfile);
    }
    // reset the interface
    renderWindow->ResetInterface();
    } // RenderController::vertexSizeChanged()
